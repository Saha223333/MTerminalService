...
Begin 
  ...
  Memo1.Lines.Add('Заливка показаний');
  Memo1.Lines.Add('Папка '+ Path);
  Memo1.Lines.Add('Время '+ DateTimeToStr(Now));
  ...
  Memo1.Lines.Add('Создан реестр №'+IntToStr(NewRoll));
  Memo1.Lines.Add('Кол-во показаний в Metering '+IntToStr(MainForm.DB.RecordCount));
	...
	Begin
	 Try
	  ...
	  MainForm.Memo1.Lines.Add('Ошибка при импорте. Account_id='+IntToStr(account_id)+' '+
							'Показания='+w_str+' '+'Дата\время='+date_m);
	  MainForm.DB.Next;
	  Continue; 
	end;
...
Memo1.Lines.Add('Начинаем разноску');   
try
  Raznoska(NewRoll);
 except on E:Exception do
  begin
	 Memo1.Lines.Add('Ошибка при разноске: '+ E.Message);
	 Exit;
  end;
end;
Memo1.Lines.Add('Разноска прошла успешно');
//Memo1.Lines.Add('Перемещаем папку в архив');
Memo1.Lines.Add(chr(13));
 {
 try
  ...
  begin
	 Memo1.Lines.Add('Ошибка при переносе папки '+ E.Message);
	 Exit;
  end;
 end;
 }
...
While not MainForm.ODSLS.Eof do
 begin
  ...
  MainForm.DB.FieldByName('Point_id').AsInteger:=point_id;
  //если счётчик РИМ-109 или РИМ-129 или РИМ-489.19 или РИМ-489.01 или РИМ-189.11, то к имени подстанции добавляем "РИМ" 
	If (MainForm.ODSLS.FieldByName('TYPE_SCH').AsInteger=148) or
		...
		MainForm.DB.FieldByName('Trans_Station').AsString:='РИМ '//+name_tp
...
If (MainForm.ODSLS.FieldByName('TYPE_SCH').AsInteger=148) or
 ...
 //если счётчик типа РИМ-109 или РИМ-129 или РИМ-489.19 или РИМ-189.11 или РИМ-489.01,
 ...
 While not MainForm.ODSDatch_m.EOF do
  Begin
    ...
    //запоминаем номер счётчика из таблицы ДДМ-ов
	 ...
	 //блок заливки для счетчиков типа РИМ-109 и РИМ-129
	 ...
	 MainForm.DB.FieldByName('Possibility').AsInteger:=Possibility;
		...
		try
		 ...
		 MainForm.Memo1.Lines.Add('Ошибка экспорта. Номер счетчика '+number);
		 MainForm.ODSLS.Next; 
		 Continue;  
		end;
	...
	begin
	 try
	  ...
	  ImportThread(s);
		...
		on E:Exception do 
		 Begin
			Memo1.Lines.Add('Ошибка заливки показаний: '+ E.Message);
			Continue;
		 End;
	 end;
	end;
...
If ParamCount<>0 then //если запускаем программу с параметром -i это значит, что после импорта показаний программа завершит работу 
	 If ParamStr(1)='-i' then	 //и экспорта не будет
		ShowMessage('Программа запущена в ручном режиме: произойдёт только слив показаний! Нажиме ОК для продолжения.');
{======Предполагаемый алгоритм работы====
1.Техники в конце дня через батник на терминале копируют свою базу с показаними в папку на главном компе 
2.По желанию техники забирают свежую версию базы данных (при этом теряя все накопленные данные)
3.Один раз в сутки программа запускается через планировщик задач
4.Сразу после запуска программа сканирует папку с показаниями, сброшенными с терминалов (если такие имеются).	
  Тут же после слива показаний (если такие были), программа обновляет эталонную базу на главном компе
}
...
DirLB.Directory:='C:\MTerminalService\';
 ...
 if DirLB.Count = 0 then Application.Terminate;
  ...
  If ParamCount<>0 then //если запускаем программу с параметром -i это значит, что после импорта показаний программа завершит работу 
	 If ParamStr(1)='-i' then	 //и экспорта не будет
		begin
		 ShowMessage('Программа завершила работу.');
		 Exit;
		end;
